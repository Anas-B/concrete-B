...
pub struct TreepbsKey(pub concrete_shortint::treepbs::TreepbsKey);
...
impl TreepbsKey {
    ...
    /// let ct_res = treepbs_key.block_mul_treepbs_with_multivalue(&sks, &ct_left, &ct_right, 0);
    ///
    /// // Decrypt
    /// let res = cks.decrypt(&ct_res);
    ...
    ///
    /// // Compute homomorphically a multiplication
    /// let ct_res = treepbs_key.mul_treepbs_with_multivalue(&sks, &mut ctxt_1, &ctxt_2);
    ///
    /// // Decrypt
    /// let res = cks.decrypt(&ct_res);
    /// assert_eq!((clear_1 * clear_2) % 256, res);
